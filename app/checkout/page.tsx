  ...
  async function placeOrder(ev: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/create-payment", {
        ...
        body: JSON.stringify({
          provider,
          // Send PKR — SahulatPay's gateway charges in Pakistani Rupees.
          // Sending the USD value as-is would charge $97 as Rs97 instead of ~Rs27,160.
    ...
    pollRef.current = window.setInterval(async () => {
      ...
      try {
        ...
        if (data.paymentStatus === "paid") {
          ...
          cart.clear();
          window.setTimeout(() => router.push("/account"), 1500);
