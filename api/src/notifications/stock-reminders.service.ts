...
import type { StockItemRow } from "../stock/stock.types";
import { daysLeft, normalizeStockItem, STOCK_STATUS_LABELS } from "../stock/stock.util";
...
@Injectable()
export class StockRemindersService {
  private readonly logger = new Logger(StockRemindersService.name);
  ...
  /**
   * Scheduled daily run, replacing the Vercel cron. Fires at 08:00 server time.
   * Set the Render service timezone (TZ env) to match the old Asia/Karachi 08:00
   * if exact timing matters.
   */
  @Cron(CronExpression.EVERY_DAY_AT_8AM, { name: "stock-expiry-reminders" })
  async scheduled() {
    try {
      const result = await this.run();
      this.logger.log(`stock-expiry-reminders ran: checked=${result.checked} sent=${result.reminders.filter((r) => r.sent).length}`);
    } catch (err) {
      this.logger.error(`stock-expiry-reminders failed: ${(err as Error).message}`);
    }
  }

  /** The actual reminder sweep. Returns a summary (also used by the manual trigger). */
  async run() {
    ...
    for (const item of items) {
      ...
      try {
        ...
        await this.email.sendEmail({ to: item.contact_email, subject: email.subject, text: email.text, html: email.html });
    ...
    return { ok: true, checked: items.length, reminders: results };
  }
}
